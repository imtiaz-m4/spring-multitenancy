...
/**
 * {@link DocumentMatcher} backed by {@link Environment#getActiveProfiles()}. A YAML document may define a
 * "spring.profiles" element as a comma-separated list of Spring profile names, optionally negated using the {@code !}
 * character. If both negated and non-negated profiles are specified for a single document, at least one non-negated
 * profile must match and no negated profiles may match.
 * <p>
 * Note (Imtiaz): Copied from spring v1.4.x source tree (spring-boot: org.springframework.boot.yaml) as it is discontinued in 2.0.x series.
 * 
 * @author Dave Syer
 * @author Matt Benson
 * @author Phillip Webb
...
public class SpringProfileDocumentMatcher implements DocumentMatcher {
	...
	public MatchStatus matches(Properties properties) {
		...
		if (StringUtils.hasLength(negative)) {
			...
			if (activeProfilesMatcher.matches(properties) == MatchStatus.FOUND) return MatchStatus.NOT_FOUND;
			if (StringUtils.isEmpty(positive)) return MatchStatus.FOUND;
	...
	private DocumentMatcher getActiveProfilesDocumentMatcher() {
		return this.activeProfiles.length == 0 ? new EmptyProfileDocumentMatcher()
				: new ActiveProfilesDocumentMatcher(new HashSet<>(Arrays.asList(this.activeProfiles)));
	}
	...
	private static abstract class AbstractProfileDocumentMatcher implements DocumentMatcher {

		@Override
		public final MatchStatus matches(Properties properties) {
			if (!properties.containsKey(SPRING_PROFILES)) return MatchStatus.ABSTAIN;
			Set<String> profiles = StringUtils.commaDelimitedListToSet(properties.getProperty(SPRING_PROFILES));
	...
	/**
	 * {@link AbstractProfileDocumentMatcher} that matches a document when a value in {@code spring.profiles} is also in
	 * {@code spring.profiles.active}.
	 */
	private static class ActiveProfilesDocumentMatcher extends AbstractProfileDocumentMatcher {
		...
		protected MatchStatus matches(Set<String> profiles) {
			if (profiles.isEmpty()) return MatchStatus.NOT_FOUND;
			for (String activeProfile : this.activeProfiles)
				if (profiles.contains(activeProfile)) return MatchStatus.FOUND;
	 ...
	 */
	private static class EmptyProfileDocumentMatcher extends AbstractProfileDocumentMatcher {

		@Override
		public MatchStatus matches(Set<String> profiles) {
			if (profiles.isEmpty()) return MatchStatus.FOUND;
			for (String profile : profiles)
				if (!StringUtils.hasText(profile)) return MatchStatus.FOUND;
			return MatchStatus.NOT_FOUND;
		}
